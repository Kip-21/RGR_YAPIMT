...
Symbol SyntaxAnalyzer::tokenToSymbol(const Token& t) {
    if (t.type == TokenType::KEYWORD) {
        ...
        if (w == "read") return Symbol::T_READ;
    }
    ...
    if (t.type == TokenType::DELIMITER || t.type == TokenType::OPERATION) {
        ...
        if (w == "(") return Symbol::T_LPAREN;
        if (w == ")") return Symbol::T_RPAREN;
...
string SyntaxAnalyzer::symToStr(Symbol s) {
    switch (s) {
        ...
        case Symbol::T_READ: return "read"; case Symbol::T_LPAREN: return "("; case Symbol::T_RPAREN: return ")";
...
SyntaxAnalyzer::SyntaxAnalyzer(const vector<Token>& tks, const ConstTable& rw, const ConstTable& delims, VarTable& vt)
    ...
    // Program
    addState(1, {Symbol::T_INT, Symbol::T_FLOAT, Symbol::T_ID, Symbol::T_READ, Symbol::T_EOF}, 2, false, false, false, true);
    addState(2, {}, S_StmtList, false, false, true, true);

    // StmtList
    addState(3, {Symbol::T_INT, Symbol::T_FLOAT, Symbol::T_ID, Symbol::T_READ}, 5, false, false, false, false);
    ...
    // Read Statement
    addState(107, {Symbol::T_READ}, 108, true, false, false, true);
    addState(108, {Symbol::T_LPAREN}, 109, true, false, false, true);
    addState(109, {Symbol::T_ID}, 110, true, false, false, true);
    addState(110, {Symbol::T_RPAREN}, 111, true, false, false, true);
    addState(111, {Symbol::T_SEMI}, 0, true, false, true, true, Action::DO_READ);
...
vector<string> SyntaxAnalyzer::parse() {
    ...
    while (true) {
        ...
        if (row.action == Action::DO_ASSIGN) {
            ...
            poliz.push_back("АЭМ");
        } else if (row.action == Action::DO_READ) {
            poliz.push_back("ЧТЕНИЕ");
        }
